...
type Actions = 'ADD_STUDENT' | 'LIST_STUDENTS' | 'ENROLL_STUDENT' | 'REMOVE_STUDENT' | 'PAY_FEES' | 'SHOW_BALANCE' | 'SHOW_STATUS';
...
const {calltoactions} = await inquirer.prompt([
   { 
    ...
    choices: [
      ...
      filter: (val:any) => val.toUppercase().replace('','_')

    },
  
] as QuestionCollection<{ calltoactions:Actions}>
)
switch(calltoactions){
    ...
    const answers = await inquirer.prompt([{
        ...
        validate: (val:any) => Boolean(val) || 'Please enter student name'
    },
    {
     type:'input',
     name:'Student ID',
     message: 'Enter student ID',
     validate: (val:any ) => val.toString(val) ==='5'|| 'Please enter 5 digit Student ID'
